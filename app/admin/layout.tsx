...
import {
  ...
  Settings, LogOut, Menu, X, ChevronLeft, Shield, MessagesSquare,
} from "lucide-react";
...
function AdminSidebar({ collapsed, onToggle, mobileOpen, onMobileToggle }: { collapsed: boolean; onToggle: () => void; mobileOpen: boolean; onMobileToggle: () => void }) {
  const pathname = usePathname();
  const { logout } = useAuth();

  useEffect(() => {
    const handleResize = () => {
      if (mobileOpen && window.innerWidth >= 1024) onMobileToggle();
    };
  ...
  return (
    <>
      <AnimatePresence>
        {mobileOpen && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onMobileToggle} className="fixed inset-0 z-40 bg-black/40 lg:hidden" />}
      </AnimatePresence>
      <AnimatePresence>
        {mobileOpen && (
          ...
          <div className="flex items-center justify-between px-4 md:px-6 lg:px-8 h-14 lg:h-16">
            <div className="flex items-center gap-2">
              <button
                ...
                className="lg:hidden flex items-center justify-center w-8 h-8 rounded-lg text-muted-foreground hover:bg-background-soft transition-colors -ml-1"
